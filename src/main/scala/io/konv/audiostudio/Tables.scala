...
trait Tables {
  ...
  class AlbumRecord(_tableTag: Tag) extends profile.api.Table[AlbumRecordRow](_tableTag, "album_record") {
    ...
    lazy val albumFk = foreignKey("album_record_album_id_fkey", albumId, Album)(r => Rep.Some(r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
    /** Foreign key referencing Record (database name album_record_record_id_fkey) */
    lazy val recordFk = foreignKey("album_record_record_id_fkey", recordId, Record)(r => Rep.Some(r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
  ...
  class Record(_tableTag: Tag) extends profile.api.Table[RecordRow](_tableTag, "record") {
    ...
    lazy val artistFk = foreignKey("record_artist_id_fkey", artistId, Artist)(r => Rep.Some(r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
    /** Foreign key referencing Genre (database name record_genre_id_fkey) */
    lazy val genreFk = foreignKey("record_genre_id_fkey", genreId, Genre)(r => Rep.Some(r.id), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.SetNull)
  }
