...
case class RecordTab(id: Int, title: String, price: Int, date: Date, artist: String)
...
class RecordTabController(table: TableView[RecordTab],
  ...
  override def update(): Unit = {
    implicit val getResult = GetResult(r => RecordTab(r.<<, r.<<, r.<<, r.<<, r.<<))
    val query = sql"""
                  SELECT record.id, record.title, record.price, record.released_date, artist.name
